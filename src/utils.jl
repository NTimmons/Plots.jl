...
unzip(xy::AVec{SVector{2,T}}) where {T}  = T[t[1] for t in xy], T[t[2] for t in xy]
unzip(xy::SVector{2,T}) where {T}        = T[xy[1]], T[xy[2]]

unzip(xyz::AVec{SVector{3,T}}) where {T} = T[t[1] for t in xyz], T[t[2] for t in xyz], T[t[3] for t in xyz]
unzip(xyz::SVector{3,T}) where {T}       = T[xyz[1]], T[xyz[2]], T[xyz[3]]

unzip(xyuv::AVec{SVector{4,T}}) where {T} = T[t[1] for t in xyuv], T[t[2] for t in xyuv], T[t[3] for t in xyuv], T[t[4] for t in xyuv]
unzip(xyuv::SVector{4,T}) where {T}       = T[xyuv[1]], T[xyuv[2]], T[xyuv[3]], T[xyuv[4]]
